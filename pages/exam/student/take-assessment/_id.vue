...
<v-app>
	<v-content>
		<v-toolbar color="primary">
			...
			<v-spacer></v-spacer>
			<v-btn color="white" outline @click.prevent="submitExam">Submit</v-btn>
		</v-toolbar>
		<v-card flat>
			<v-container>
					...
					<v-tabs-items touchless>
						<v-tab-item v-for="i in questions.length" :key="i" :value="'tab-' + i">
							<v-card flat class = "card-content">
								...
								<center>
									<span v-if="questions[i-1].question_image!='null'">
									<!-- <v-img class="question-image" :src="`${baseUrl}/exams/question/${questions[i-1].question_image}`"></v-img> -->
									<v-img class="question-image" :src="`${baseUrl}/${questions[i-1].question_image}`"></v-img>
									</span>
								</center>
								<div class="option-radiogroup">
									<v-radio-group v-model="answers[i-1]">
										...
										<span v-if="questions[i-1].option_5!='null'">
											...
											<v-img :src="`${baseUrl}/${questions[i-1].option_5}`"></v-img>
										</span>
										</v-flex>
										<v-flex xs12 sm12 md6>
										...
										<span v-if="questions[i-1].option_7!='null'">
											...
											<v-img :src="`${baseUrl}/${questions[i-1].option_7}`"></v-img>
										...
										<span v-if="questions[i-1].option_6!='null'">
											...
											<v-img :src="`${baseUrl}/${questions[i-1].option_6}`"></v-img>
										...
										<span v-if="questions[i-1].option_8!='null'">
											...
											<v-img :src="`${baseUrl}/${questions[i-1].option_8}`"></v-img>
...
export default {
	...
	methods: {
		...
		async submitExam() {
			await this.$axios.post(`/api/results/${this.$route.params.id}/check`,{
				data: this.answers
			})
			this.$router.push(`/exam/student/result/${this.$route.params.id}`)
		}
	}
}
</script>
